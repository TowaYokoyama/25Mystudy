...
import { View, Text, Alert, TouchableOpacity, Modal, TextInput, SafeAreaView, Image, ScrollView } from 'react-native';
...
import Swiper from 'react-native-deck-swiper'; // 新しいライブラリをインポート

import Flashcard from '@/components/FlashCard'; // ファイル名に合わせて修正
...
export default function DeckDetailScreen() {
  ...
  const handleAddCard = async () => {
    const finalFrontText = deck_type === 'image' ? null : frontText;
  ...
  const handleDeleteCard = (cardIndex: number) => {
    ...
    Alert.alert("カードの削除", "このカードを本当に削除しますか？", [
      { text: "キャンセル", style: "cancel", onPress: () => swiperRef.current?.swipeBack() },
      {
        text: "削除",
        onPress: async () => {
            ...
            <View style={tw`bg-gray-900 p-6 rounded-2xl w-11/12`}>
              ...
              {deck_type === 'image' ? (
                <TouchableOpacity onPress={() => pickImage('front')} style={tw`w-full h-24 bg-gray-800 rounded-lg justify-center items-center mb-4`}>
                  {frontImage ? <Image source={{ uri: frontImage.uri }} style={tw`w-full h-full rounded-lg`} /> : <Text style={tw`text-gray-400`}>表面の画像を選択</Text>}
                </TouchableOpacity>
              ) : (
              ...
              <TouchableOpacity onPress={() => pickImage('back')} style={tw`w-full h-24 bg-gray-800 rounded-lg justify-center items-center mb-2`}>
                {backImage ? <Image source={{ uri: backImage.uri }} style={tw`w-full h-full rounded-lg`} /> : <Text style={tw`text-gray-400`}>裏面の画像（任意）</Text>}
              </TouchableOpacity>
      ...
      <View style={tw`flex-1`}>
        {cards.length > 0 ? (
          <Swiper
            ref={swiperRef}
            cards={cards}
            renderCard={(card: Card | undefined) => {
              if (!card) return null;
              return (
                <Flashcard 
            ...
            onSwiped={ handleDeleteCard}
            cardIndex={0}
            backgroundColor={'transparent'}
            stackSize={3}
            stackSeparation={18}
           cardVerticalMargin={40}      // スタック全体の垂直マージン
            stackScale={5}    
